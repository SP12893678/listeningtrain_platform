...
export default class ScoreCaculate {
    ...
    getExamData () {
        return apiManageLearning({ type: 'get' }).then((res) => {
            ...
            const past_exams = JSON.parse(res.data.test).test
    ...
    getAverageScoreDataAll () {
        const exam_data = this.exam_data
        const average_score = this.getDefaultFormateObject()
        const first_response_rate = exam_data[0].response_rate
    ...
    getAverageScoreData (enviro_id) {
        const exam_data = this.exam_data.filter((exam) => exam.enviro_id == enviro_id)
        const average_score = this.getDefaultFormateObject()
        const first_response_rate = exam_data[0].response_rate
        exam_data.forEach((exam) => {
            average_score.accuracy.your += exam.accuracy.your
