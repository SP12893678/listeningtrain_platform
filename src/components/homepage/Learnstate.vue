    ...
    >
      <v-list-item-content>
        <v-data-table
          ...
          dense
        >
          <template #item.envirotime="{ item }">
            {{ envirotime(item) }}
          </template>
          <template #item.objecttime="{ item }">
            {{ objecttime(item) }}
          </template>
        </v-data-table>
      </v-list-item-content>
    ...
    >
      <v-list-item-content>
        ...
        >
          ...
          <template #item.que="{ item }">
            {{ practiceque(item) }}
          </template>
          <template #item.firstans="{ item }">
    ...
    computed: {
        ...
        envirotime: function () {
            return function (item) {
                const record = this.explore_record
                let count = 0
        ...
        practicetime: function () {
            return function (item) {
                const record = this.practice_record
                let count = 0
                record.forEach((record) => {
